...
// ── 기부체납 운영비/상환용 (공공주택 + 공공발코니 + 공공시설) ──
  ...
  const result = months.map((_,i) => {
    ...
    // (PF이자/원금은 상환용계좌에서 차감)
    let remain = out + midInt_est + (vatByMonthArr[i] > 0 ? vatByMonthArr[i] : 0);
  ...
  const totalOutWithFin = totalOut.map((v,i)=>v+finFeeOnlyByMonth[i]+finMidByMonth[i]+vatByMonthArr[i]+(eqRepayByMonth[i]||0));
