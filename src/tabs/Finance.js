...
// ── 기부체납 운영비/상환용 (공공주택 + 공공발코니 + 공공시설) ──
  ...
  const result = months.map((_,i) => {
    ...
    // (PF이자/원금은 상환용계좌에서 차감)
    let remain = out + midInt_est + vatOut - vatIn;
          ...
          <tbody>
            ...
            {openSections.sec2 && (<>
              ...
              {vatByMonthArr.some(v=>v!==0) && (() => {
                const vatOut_ = vatByMonthArr.map(v=>v>0?v:0);
        ...
        return (
          <div style={{ marginTop:'8px', padding:'8px 12px', borderRadius:'6px', fontSize:'11px',
            ...
            <span style={{ color:'#555', marginLeft:'12px' }}>
              (분양수입 {formatNumber(Math.round(salesTotal_))} - 사업비지출 {formatNumber(Math.round(costTotal_))} - 부가세납부 {formatNumber(Math.round(vatNetPay_))}) = {formatNumber(Math.round(lhsCheck_))}
              {' vs '}
